...
class FrameSeeder extends Seeder
{
    ...
    public function run()
    {
        // Initialize an array to hold frame IDs
        $frameIds = [];
    
        // Insert frames individually and retrieve their IDs
        $frameIds[] = DB::table('frames')->insertGetId([
            'name' => 'Classic Black', 
            'description' => 'A timeless classic frame in black.', 
            'status' => 'active', 
            'stock' => 15
        ]);
        $frameIds[] = DB::table('frames')->insertGetId([
            'name' => 'Retro Blue', 
            'description' => 'Blue retro frames for the fashion-forward.', 
            'status' => 'active', 
            'stock' => 10
        ]);
